...
SImin = .40     # Minimum sequence identity for the BRIDGING algorithm
...
def start_bridger(contigs_file, reads_file, cr_overlaps_file, rr_overlaps_file, paramdict, output=True):
    ...
    if output:
        sys.stdout.write('\n\nSCARA BRIDGER: Path group info: SNODE, ENODE, DIRECTION, NUMPATHS')
        for pinfo_group in path_info_groups:
            (sname, ename, length, numNodes, direction, SIavg, path) = pinfo_group[0]
            sdirection = 'LEFT' if direction == directionLEFT else 'RIGHT'
            sys.stdout.write('\nSCARA BRIDGER: %s %s %s %d' % (sname, ename, sdirection, len(pinfo_group)))
    ...
    if output:
        sys.stdout.write('\nSCARA BRIDGER: Discarded groups: SNODE, ENODE, DIRECTION, NUMPATHS')
        for pinfo_group in discarded_groups:
            (sname, ename, length, numNodes, direction, SIavg, path) = pinfo_group[0]
            sdirection = 'LEFT' if direction == directionLEFT else 'RIGHT'
            sys.stdout.write('\nSCARA BRIDGER: %s %s %s %d' % (sname, ename, sdirection, len(pinfo_group)))

        sys.stdout.write('\n\nSCARA BRIDGER: Remaining groups: SNODE, ENODE, DIRECTION, NUMPATHS')
    ...
    longpaths = 0
    if output:
        sys.stdout.write('\nSCARA BRIDGER FINAL PATHS: SNODE, ENODE, LENGTH, NUMNODES, DIRECTION, SIAVG')
        for (sname, ename, length, numNodes, direction, SIavg, path) in final_paths:
            sdirection = 'LEFT' if direction == directionLEFT else 'RIGHT'
            sys.stdout.write('\nSCARA BRIDGER: %s %s %d %d %s %f' % (sname, ename, length, numNodes, sdirection, SIavg))
...
def verbose_usage_and_exit():
    sys.stderr.write('briger - a scaffolding tool that briges gaps between contigs using long reads.\n')
    sys.stderr.write('\n')
