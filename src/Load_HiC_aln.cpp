...
void print_version_message_and_exit() {
  const char* versionmessage = "\nLoad_HiC_SAM version 0.1!\n";
  std::cerr << versionmessage;
  exit(0);
}

void print_help_message_and_exit() {
  const char* helpmessage = "\nHow to run Load_HiC_SAM :"
    "\nLoad_HiC_SAM -1 (--Hi-C1) <First Hi-C_SAM file> -2 (--Hi-C2) <Second Hi-C_SAM file> [-r (--reference) <Reference file>]"
    "\n    or"
    "\nLoad_HiC_SAM -c (--Hi-C) <Combined Hi-C_SAM file> [-r (--reference) <Reference file>]"
    "\n"
...
void analyzeCombinedHiC(scara::MapIdToSeq mIdToReadHiC, scara::MapIdToSeq mIdToReadReference, int refSet) {
  std::cout << "\n\nAnalyzing combined Hi-C alignments ...";
  std::cout << "\nHi-C alignments file size: " << mIdToReadHiC.size();
  if (refSet) {
...
void analyzeSeparateHiC(scara::MapIdToSeq mIdToReadHiC1, scara::MapIdToSeq mIdToReadHiC2, scara::MapIdToSeq mIdToReadReference, int refSet) {
  std::cout << "\n\nAnalyzing separate Hi-C alignments ...";
  std::cout << "\nFirst Hi-C alignments file size: " << mIdToReadHiC1.size();
  std::cout << "\nSecond Hi-C alignments file size: " << mIdToReadHiC2.size();
  if (refSet) {
    std::cout << "\nReference file size: " << mIdToReadReference.size();
  }
  else {
